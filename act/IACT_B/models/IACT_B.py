"""
IACT_B model with dual heads:
1. Joint reference head (vanilla ACT output)
2. Primitive head (primitive logits and parameters)
"""
import torch
...
class PrimitiveHead(nn.Module):
    """
    Primitive head that outputs:
    - z: primitive logits (categorical)
    - θ: primitive parameters (regression)
    """
    def __init__(self, hidden_dim, num_primitives, primitive_param_dim):
        ...
        # Primitive ID logits (categorical)
        self.primitive_id_head = nn.Linear(hidden_dim, num_primitives)
        
        # Primitive parameters (regression)
    ...
    def forward(self, hs):
        ...
        """
        primitive_logits = self.primitive_id_head(hs)
        primitive_params = self.primitive_param_head(hs)
        return primitive_logits, primitive_params


class IACT_B_VAE(nn.Module):
    """
    IACT_B model with dual heads:
    - Joint reference head (q_ref): outputs joint positions like vanilla ACT
    - Primitive head (z, θ): outputs primitive logits and parameters
    """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names,
                 num_primitives=6, primitive_param_dim=8):
        """
        Args:
        ...
        # Joint reference head (vanilla ACT output)
    ...
    def forward(self, qpos, image, effort, env_state, actions=None, is_pad=None):
        ...
        Returns:
            q_ref: (batch, num_queries, state_dim) - joint reference positions
        ...
        # Dual heads: joint reference + primitive
        q_ref = self.action_head(hs)  # (batch, num_queries, state_dim)
        primitive_logits, primitive_params = self.primitive_head(hs)
        is_pad_hat = self.is_pad_head(hs)
        
        return q_ref, primitive_logits, primitive_params, is_pad_hat, [mu, logvar]
...
def build(args):
    ...
    num_primitives = getattr(args, 'num_primitives', 6)  # Default: 6 primitive types
