"""
IACT (Impedance ACT) model with PrimitiveDecoderHead
"""
import torch
...
class PrimitiveDecoderHead(nn.Module):
    """
    High-level component inside ACT that outputs:
    - p(z) = primitive ID distribution (categorical logits)
    - θ = primitive parameters
    """
    def __init__(self, hidden_dim, num_primitives, primitive_param_dim):
        """
        Args:
            hidden_dim: Hidden dimension from transformer
            num_primitives: Number of primitive types (e.g., move, grasp, insert, etc.)
            primitive_param_dim: Dimension of parameters for each primitive
        """
        super().__init__()
        self.num_primitives = num_primitives
        self.primitive_param_dim = primitive_param_dim
        
        # Output primitive ID distribution logits
        self.primitive_id_head = nn.Linear(hidden_dim, num_primitives)
        
        # Output primitive parameters
        self.primitive_param_head = nn.Linear(hidden_dim, primitive_param_dim)
    
    def forward(self, hs):
        """
        Args:
            hs: Transformer hidden states (batch, num_queries, hidden_dim)
        Returns:
            primitive_logits: (batch, num_queries, num_primitives) - logits for primitive ID distribution
            primitive_params: (batch, num_queries, primitive_param_dim) - parameters for each primitive
        """
        primitive_logits = self.primitive_id_head(hs)  # (batch, num_queries, num_primitives)
        primitive_params = self.primitive_param_head(hs)  # (batch, num_queries, primitive_param_dim)
        return primitive_logits, primitive_params


class IACTVAE(nn.Module):
    """
    IACT model with PrimitiveDecoderHead
    Based on DETRVAE but outputs primitives instead of direct actions
    """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, 
                 num_primitives=8, primitive_param_dim=14):
        """
        Args:
        ...
        self.primitive_decoder = PrimitiveDecoderHead(
            hidden_dim=hidden_dim,
            num_primitives=num_primitives,
            primitive_param_dim=primitive_param_dim
        )
        
        # Keep is_pad head for compatibility
    ...
    def forward(self, qpos, image, effort, env_state, actions=None, is_pad=None):
        ...
        Returns:
            primitive_logits: (batch, num_queries, num_primitives) - primitive ID logits
            primitive_params: (batch, num_queries, primitive_param_dim) - primitive parameters
        ...
        # Decode primitives
        primitive_logits, primitive_params = self.primitive_decoder(hs)
        is_pad_hat = self.is_pad_head(hs)
        
        return primitive_logits, primitive_params, is_pad_hat, [mu, logvar]
...
def build(args):
    """Build IACT model"""
    state_dim = 7  # TODO: make configurable
    ...
    num_primitives = getattr(args, 'num_primitives', 8)  # Default: 8 primitive types
    primitive_param_dim = getattr(args, 'primitive_param_dim', 14)  # Default: 14D params
