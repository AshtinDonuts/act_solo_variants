...
import numpy as np
import torch
from .models import build_impedance_ACT_model

import IPython
e = IPython.embed

def get_args_parser():
    """ standard Args parser inherited from ACT solo codebase """
...
def build_IACT_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('Impedance ACT training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()

    for k, v in args_override.items():
        setattr(args, k, v)

    model = build_impedance_ACT_model(args)
    model.cuda()
